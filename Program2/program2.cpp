...
Employee emp_populate(vector<string> tokens){//function to populate a vector of our Employee object using the vector of strings from our previous function
    int i = stoi(tokens[2]);//converting one element of our vector of strings to an integer 
    Employee e(tokens[1], tokens[0], i, tokens[3], tokens[4]);//filling our vector of employees
    return e;//returning our vector of employees
}

Student stu_populate(vector<string> tokens){//function to populate a vector of our Student object using the vector of string from our first function
    int i = stoi(tokens[2]);//converting two our elements from the vector of strings into integers
    int c = stoi(tokens[3]);
    Student s(tokens[1], tokens[0], i, c);//filling our vector of students
    return s;//returning our vector of students
}

UniversityClass cla_populate(vector<string> tokens){//function to populate a vector of our UniversityClass object using the vector of strings from the first function
    int cr = stoi(tokens[9]);//converting one element of our vector of strings into an integer
    UniversityClass c(tokens[0], tokens[1], tokens[6], tokens[7], tokens[8], cr);//filling our vector of University Classes
    return c;//returning our vector of University Classes
}
...
int main(void){
    ...
    while(getline(inpfile1, line1)){//while loop to loop through each line in our first input file 
        ...
        emp.push_back(emp_populate(tokens1));//calling our function to populate our vector of employees
    }
    ...
    while(getline(inpfile2, line2)){//while loop to loop through each line in our second input file 
        ...
        stu.push_back(stu_populate(tokens2));//calling our function to populate our vector of students
    }
    ...
    while(getline(inpfile3, line3)){//while loop to loop through each line in our third input file
        ...
        cla.push_back(cla_populate(tokens3));//calling our function to populate our vector of University Classes
    }
